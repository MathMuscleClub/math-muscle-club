...
async function handleImageUpload(
  ...
  if (preconvertedTex && preconvertedTex.trim()) {
    ...
    // TeXをGitHubに保存後、GitHub Actions ワークフローをトリガーしてPDFを自動ビルド
    triggerPdfBuild([filePath]).catch(() => {});
    const manifest = await readManifest();
    const entry: ManifestItem = {
      id,
      year: Number(yearStr),
  ...
  // Gemini生成のTeXをGitHubに保存後、GitHub Actions ワークフローをトリガーしてPDFを自動ビルド
  triggerPdfBuild([filePath]).catch(() => {});
...
async function handleNewAnswerUpload(
  ...
  if (isPdf) {
    ...
    // TeXファイルをGitHubに保存後、GitHub Actions ワークフローをトリガーしてPDFを自動ビルド
    triggerPdfBuild([filePath]).catch(() => {});
  }
...
/**
 * GitHub Actions の build-exam-pdf.yml ワークフローをトリガーして
 * 指定された TeXファイルを即座にPDFにビルドさせる。
 * GITHUB_TOKEN が設定されていない場合や失敗した場合はエラーを無視する
 * （push トリガーによるフォールバックがあるため）。
 */
async function triggerPdfBuild(texPaths: string[]): Promise<void> {
  const token = Deno.env.get("GITHUB_TOKEN");
  if (!token) return; // トークンなしの場合は push トリガーに任せる

  const repository = gitHubRepository();
  const branch = gitHubBranch();
  const workflowId = "build-exam-pdf.yml";
  const url = `https://api.github.com/repos/${repository}/actions/workflows/${workflowId}/dispatches`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`GitHub Actions トリガー失敗 (${response.status}): ${text.slice(0, 200)}`);
    } else {
      console.log(`GitHub Actions ワークフローをトリガーしました: ${texPaths.join(", ")}`);
    }
  } catch (error) {
    // ネットワークエラーなどは無視（push トリガーにフォールバック）
    console.warn("GitHub Actions トリガー中にエラー:", error);
  }
}
