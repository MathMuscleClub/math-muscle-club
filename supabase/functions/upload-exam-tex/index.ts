...
async function handlePdfUpload(
  ...
  const replacedItems = findOwnedAnswerSlotItems(manifest.items, yearStr, problemGroup, user, email);
  await deleteAnswerFilesForItems(replacedItems, `Replace ${yearStr} ${problemGroup} answer PDF by ${username}`);

  const nextManifest = upsertAnswerSlotManifestItem(manifest.items, entry, user, email);
  await putGitHubFile(
    "exam-submissions.json",
    `${JSON.stringify(nextManifest, null, 2)}\n`,
...
async function handleImageUpload(
  ...
  if (preconvertedTex && preconvertedTex.trim()) {
    ...
    const replacedItems = findOwnedAnswerSlotItems(manifest.items, yearStr, problemGroup, user, email);
    await deleteAnswerFilesForItems(replacedItems, `Replace ${yearStr} ${problemGroup} answer TeX by ${username}`);
...
async function handleNewAnswerUpload(
  ...
  const replacedItems = findOwnedAnswerSlotItems(manifest.items, yearStr, problemGroup, user, email);
  await deleteAnswerFilesForItems(replacedItems, `Replace ${yearStr} ${problemGroup} answer by ${username}`);
...
async function deleteSubmission(
  ...
  const answerFilePath = getAnswerFilePath(target);
  if (!answerFilePath) {
    return errorResponse("削除する解答ファイルが見つかりません。");
  }

  await deleteGitHubFileIfExists(answerFilePath, `Delete exam answer by ${username}`);
...
async function updateAnswerSubmission(
  ...
  const answerTexPath = getAnswerFilePath(target) || defaultAnswerTexPath(answerPart, submissionId);
  const problemPart = findMatchingProblemPart(problemParts, answerPart);
  const problemTexPath = problemPart
    ? sharedProblemTexPath(problemPart)
...
function upsertAnswerSlotManifestItem(
  items: ManifestItem[],
  entry: ManifestItem,
  user: { id?: string },
  email: string,
  ...
  return [
    entry,
    ...items.filter((item) => {
      if (String(item.id || "") === String(entry.id || "")) return false;
      return !isOwnedAnswerSlotItem(item, String(entry.year || ""), String(entry.problemGroup || ""), user, email);
    }),
  ];
}

function findOwnedAnswerSlotItems(
  items: ManifestItem[],
  year: string,
  problemGroup: string,
  user: { id?: string },
  ...
  return items.filter((item) => isOwnedAnswerSlotItem(item, year, problemGroup, user, email));
}

function isOwnedAnswerSlotItem(
  item: ManifestItem,
  year: string,
  problemGroup: string,
  user: { id?: string },
  ...
  return String(item.status || "") === "submitted"
    && Boolean(getAnswerFilePath(item))
    && String(item.year || "") === String(year || "")
    && cleanProblemGroup(item.problemGroup) === cleanProblemGroup(problemGroup)
    && canManageManifestItem(item, user, email);
}

async function deleteAnswerFilesForItems(items: ManifestItem[], message: string) {
  const paths = [...new Set(items.map(getAnswerFilePath).filter(Boolean))];
  for (const path of paths) {
    await deleteGitHubFileIfExists(path, message);
  }
}
...
function getAnswerFilePath(item: ManifestItem) {
