...
$A = ¥mathbb{C}[T]$ を複素数体 $¥mathbf{C}$ 上の1変数多項式環とし，$K = ¥mathbb{C}(T)$ をその分数体とする．$B$ を $¥mathbb{C}$ 上の3変数多項式環 $¥mathbb{C}[X,Y,Z]$ のイデアル $I = (XY,YZ,ZX)$ による商環 $¥mathbb{C}[X,Y,Z]/I$ とする．また $¥mathbb{C}$ 上の環準同型 $f : A ¥to B$ を $f(T) = ¥overline{X+Y+Z}$ で定める．ただしここで，$¥overline{X+Y+Z}$ は $B$ における $X+Y+Z$ の同値類である．以下の問に答えよ．
