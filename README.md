...
- `exams/{年度}/{問題区分}/{問題番号}/submissions/{提出ID}/problem.tex`: 提出された問題文TeX
- `exams/{年度}/{問題区分}/{問題番号}/submissions/{提出ID}/answer.tex`: 提出された解答TeX
...
提出フォームでは、問題文TeXと解答TeXを別ファイルとして選びます。年度・分野・問題区分・問題番号はフォームで入力せず、TeX内のマクロから読み取ります。
...
1つのTeXに複数の問題や解答を入れる場合は、各ブロックの先頭に同じメタ情報マクロを置きます。
...
PDFレンダラーをまだ用意しない場合、`render-exam-pdf` の環境変数は `ALLOWED_ORIGIN` だけでも構いません。サイト側は印刷画面の一時生成にフォールバックします。
